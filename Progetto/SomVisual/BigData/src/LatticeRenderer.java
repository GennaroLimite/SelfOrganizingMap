...
public class LatticeRenderer extends JPanel {
	...
	public void render(SOMLattice lattice, int iteration) {
		...
		for (int x=0; x<lattice.getWidth(); x++) {
			for (int y=0; y<lattice.getHeight(); y++) {
				r = (float)((Double)lattice.getNode(x,y).getVector().get(0)).doubleValue();
				g = (float)((Double)lattice.getNode(x,y).getVector().get(1)).doubleValue();
				b = (float)((Double)lattice.getNode(x,y).getVector().get(2)).doubleValue();
	...
	private void panelMouseMoved(java.awt.event.MouseEvent evt) {
		...
		g2.setFont(arialFont);
		float r = (float)((Double)lattice.getNode(cellX,cellY).getVector().get(0)).doubleValue();
		float g = (float)((Double)lattice.getNode(cellX,cellY).getVector().get(1)).doubleValue();
		float b = (float)((Double)lattice.getNode(cellX,cellY).getVector().get(2)).doubleValue();
